-- Auto-generated colorscheme: Black Metal (Mayhem)
local M = {}
...
function M.load()
  ...
  local colors = require('black_metal_mayhem.palette')
  local highlights = {}

  -- Collect all highlight groups
  for _, mod in ipairs({ 'editor', 'syntax', 'treesitter', 'lsp', 'plugins' }) do
    local ok, hl_mod = pcall(require, 'black_metal_mayhem.highlights.' .. mod)
    if ok then
