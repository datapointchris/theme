-- Auto-generated colorscheme: Gruvbox Dark Medium
local M = {}
...
function M.load()
  ...
  local colors = require('gruvbox_dark_medium.palette')
  local highlights = {}

  -- Collect all highlight groups
  for _, mod in ipairs({ 'editor', 'syntax', 'treesitter', 'lsp', 'plugins' }) do
    local ok, hl_mod = pcall(require, 'gruvbox_dark_medium.highlights.' .. mod)
    if ok then
