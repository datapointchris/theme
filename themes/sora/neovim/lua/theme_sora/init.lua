-- Auto-generated colorscheme: Sora
local M = {}
...
function M.load()
  ...
  vim.g.colors_name = "theme-sora"
  vim.o.termguicolors = true
  vim.o.background = "dark"

  local colors = require("theme_sora.palette")
  local highlights = {}

  -- Collect all highlight groups
  for _, mod in ipairs({ "editor", "syntax", "treesitter", "lsp", "plugins" }) do
    local ok, hl_mod = pcall(require, "theme_sora.highlights." .. mod)
    if ok then
  ...
  local ok, overrides = pcall(require, "theme_sora.overrides")
