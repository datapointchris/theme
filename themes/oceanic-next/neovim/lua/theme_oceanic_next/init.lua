-- Auto-generated colorscheme: Oceanic Next
local M = {}
...
function M.load()
  ...
  vim.g.colors_name = "theme-oceanic-next"
  vim.o.termguicolors = true
  vim.o.background = "dark"

  local colors = require("theme_oceanic_next.palette")
  local highlights = {}

  -- Collect all highlight groups
  for _, mod in ipairs({ "editor", "syntax", "treesitter", "lsp", "plugins" }) do
    local ok, hl_mod = pcall(require, "theme_oceanic_next.highlights." .. mod)
    if ok then
  ...
  local ok, overrides = pcall(require, "theme_oceanic_next.overrides")
