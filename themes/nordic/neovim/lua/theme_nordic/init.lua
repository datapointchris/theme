-- Auto-generated colorscheme: Nordic
local M = {}
...
function M.load()
  ...
  vim.g.colors_name = "theme-nordic"
  vim.o.termguicolors = true
  vim.o.background = "dark"

  local colors = require("theme_nordic.palette")
  local highlights = {}

  -- Collect all highlight groups
  for _, mod in ipairs({ "editor", "syntax", "treesitter", "lsp", "plugins" }) do
    local ok, hl_mod = pcall(require, "theme_nordic.highlights." .. mod)
    if ok then
  ...
  local ok, overrides = pcall(require, "theme_nordic.overrides")
