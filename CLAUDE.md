...
**`generate-all.sh` does not run this.** It maps generators to filenames under
`lib/generators/`, and this one is Python, takes a theme *directory* rather than
a `theme.yml`, and writes a tree rather than a file. So "regenerate broadly"
below covers every app config and reaches no colorscheme: an improvement here
lands on a theme only when that theme is regenerated by hand, and
`tests/generators.bats`'s regenerate-and-diff invariant does not cover `neovim/`
either.
