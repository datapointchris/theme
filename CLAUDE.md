...
- **Spell American, except where you are quoting upstream's own slot name.** The
  repo spells identifiers and prose American and the codespell hook gates it, so
  the British form in a key or a sentence is a finding. A comment naming the
  upstream slot a value came from is a citation and keeps upstream's spelling —
  everforest calls its slots `grey1` and `grey2`, and `gray1` would name nothing.
  `scripts/check-spelling.sh` is what tells the two apart, because neither the
  hook nor a `\b`-anchored search can see a digit-suffixed token like `grey1`.
...
`scripts/check-spelling.sh` finds British spellings the commit gate cannot see.
Run it before a release and after transcribing a plugin palette.

**The gate has two blind spots and they overlap.** codespell tokenizes on its
default word regex, which keeps `display-panes-colour` and `grey1` whole — there
is no word boundary between a letter and a digit, so an `rg` pattern anchored on
`\b` misses the same two. codespell also refuses to open a dot-prefixed file
without `-H`, so `.codespellrc` and `.pre-commit-config.yaml` are outside it. The
script widens the token regex to letters only and passes `-H`, which reaches all
three classes in one call.

**Widening it is what makes the settled list necessary.** A British spelling that
is genuinely correct then surfaces on every run, so `SETTLED` declares each one
with its reason and the run reports them as a count rather than as findings. It
holds the five places the spelling deliberately stays: tmux's three `*-colour`
option names, `align=centre`, everforest's `grey1`/`grey2`, hex fragments the
widened regex reads as words, and a backticked citation of a spelling being
discussed. Anything not matching one is printed with its source line and the
script exits non-zero.

`--list-settled` prints the settled findings too, which is how you check a
pattern is still earning its place rather than absorbing something new.
...
| `scripts/check-spelling.sh` | Finds British spellings the pre-commit hook cannot see, and declares the ones that stay; see Checking Spelling |
