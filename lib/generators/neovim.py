...
# dependencies = ["pyyaml"]
# ///
"""Generate a Neovim colorscheme from theme.yml.

Creates a standalone Neovim colorscheme plugin that can be loaded via Lazy.nvim.

Unlike every other generator this writes a directory tree rather than a single
file, so its second argument is the output *directory*. The argument order is
the same as the rest of them, which is what lets generate-all.sh drive it.
"""
...
def colorscheme_name_for(meta: dict, slug: str) -> str:
    ...
    machine that does not have the plugin, so it has to be addressable *without*
    displacing the plugin on a machine that does.

    A theme with no plugin behind it keeps its bare id: that colorscheme is the
    theme rather than a stand-in for one, and renaming it now would orphan the
    entries already in history.jsonl.
    """
    if meta.get("neovim_colorscheme_source") == "plugin":
        return f"theme-{slug}"
    return slug
...
def generate_colorscheme(theme_path: Path, output_dir: Path) -> None:
    ...
    colorscheme = colorscheme_name_for(meta, slug)
    module_name = slug_to_module(colorscheme)
    ...
    files = {
        lua_dir / "init.lua": generate_init_lua(module_name, name, colorscheme),
        ...
        colors_dir / f"{colorscheme}.lua": generate_colors_lua(module_name),
    }
...
def main():
    if len(sys.argv) < 2:
        print("Usage: neovim.py <theme.yml> [output-dir]")
        print("  output-dir defaults to a neovim/ beside the theme.yml")
        sys.exit(1)
    ...
    output_dir = Path(sys.argv[2]) if len(sys.argv) > 2 else theme_path.parent / "neovim"
